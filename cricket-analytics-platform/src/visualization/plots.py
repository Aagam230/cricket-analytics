...
def plot_win_probability_timeline(
    timeline_df: pd.DataFrame, batting_team: str, bowling_team: str, target: int
) -> go.Figure:
    """
    Ball-by-ball win-probability line chart for a single second-innings
    chase -- the flagship visual for the in-match ("live") win-probability
    model (see `src/models/live_win_probability.py`). Replays a real
    historical chase and shows how the model's predicted win probability
    for the batting (chasing) team moved as the innings progressed, with
    wicket-fall moments marked.

    Args:
        timeline_df: DataFrame with one row per ball, columns:
            `balls_bowled`, `batting_team_win_probability` (0-1), and
            `is_wicket` (bool) -- see the "Live Win Probability" Streamlit
            page for how this is constructed match-by-match using
            `src.models.live_win_probability.predict_live_win_probability`.
        batting_team: Name of the chasing team (for the trace label).
        bowling_team: Name of the defending team (for the trace label).
        target: The chase target, shown in the chart title.

    Returns:
        Plotly Figure -- a filled area line at 50% shown as a reference,
        the batting team's win probability curve, and markers at each
        wicket-fall ball.
    """
    fig = go.Figure()

    fig.add_hline(y=50, line_dash="dot", line_color="gray", opacity=0.6)

    fig.add_trace(go.Scatter(
        x=timeline_df["balls_bowled"],
        y=timeline_df["batting_team_win_probability"] * 100,
        mode="lines",
    ...
    wickets = timeline_df[timeline_df["is_wicket"]]
    if not wickets.empty:
        fig.add_trace(go.Scatter(
            x=wickets["balls_bowled"],
            y=wickets["batting_team_win_probability"] * 100,
            mode="markers",
            name="Wicket",
            marker=dict(color=SECONDARY_COLOR, size=10, symbol="x"),
        ))

    fig.update_layout(
        xaxis=dict(title="Balls Bowled (2nd Innings)"),
        yaxis=dict(title=f"{batting_team} Win Probability (%)", range=[0, 100]),
    )
    return _apply_standard_layout(
        fig, f"{batting_team} chasing {target} vs {bowling_team} — Win Probability Timeline", height=460
    )

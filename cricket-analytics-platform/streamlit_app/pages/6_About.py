...
Section above documents the honest, close-to-chance performance of
predicting a match winner *before* it starts. That naturally raises the
question: is the low performance a modeling problem, or an information
problem? To find out, a second model was built that predicts win
probability *during* a second-innings chase, using the live match state
(target, current score, wickets lost, overs remaining) rather than
pre-match information.

The result: **test ROC-AUC of 0.876**, compared to 0.538 for the
pre-match model — a large, genuine gap that confirms the hypothesis: most
of what decides an IPL match isn't knowable before the match starts, but
once the game is underway, the state of the game itself is a strong
predictor. See the **Live Win Probability** page to try it — either
replay a real historical chase ball-by-ball, or enter a hypothetical
match state for an instant probability estimate. Full methodology and
results are in `docs/MODEL_CARD.md`, Part 2.
